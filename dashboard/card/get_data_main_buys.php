  ...
  <table width="100%" border="0" class="table table-bordered" id="show_card">
    ...
    <?php
      if($_SESSION['uclass'] !=3){ 
        $getcard = $getdata->my_sql_select(NULL,"card_info"," card_customer_work_group=".$_SESSION['uwork_id']." AND card_status = '' AND card_type='1'  ORDER BY card_insert");
      }else{
        $getcard = $getdata->my_sql_select(NULL,"card_info"," card_status = ''AND card_type='1' ORDER BY card_insert");
      }
      ...
      ?>
          <tr style="font-weight:bold;" id="<?php echo @$showcard->card_key;?>">
            ...
            <td align="right"><a href="?p=card_create_buy_detail&key=<?php echo @$showcard->card_key;?>" title="แก้ไข" class="btn btn-xs btn-info"><i
...
<?php
	   ...
	   echo '<div class="alert alert-info alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>ไม่พบข้อมูลใบสั่งซื้อ/เคลมที่ไม่สมบูรณ์</div>';
   }
?>