<div class="row">
  <div class="col-lg-12">
    <h1 class="page-header"><i class="fa fa-edit fa-fw"></i> สั่งซื้อ</h1>
  </div>
</div>
<ol class="breadcrumb">
  <li><a href="index.php">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="form-group">
            <label for="card_code">รหัสการสั่งซื้อ</label>
...
<script language="javascript">
 var socket = io('//127.0.0.1:8080');
  ...
  function deleteCard(cardkey) {
    if (confirm('คุณต้องการลบใบสั่งซื้อ/เคลมนี้ใช่หรือไม่ ?')) {
  ...
  $(document).ready(() => { 
        ...
        $.post('card/save_card_info.php', queryString, (response) => {
          ...
          if(obj.satuts===true){
            ...
            setTimeout(() => { 
              ...
              $('#processes').empty();
              $('#form_add').resetForm();
              window.location='?p=card_create_buy_detail&key='+obj.card_key;
    ...
    socket.on('show_card', (message) => {
      console.log(message);
      $.post( "card/get_data_main_buys.php", (data) => {
        console.log(data);
        $('#show_card').html(data);
      });
    });
